...
def generate_greeting():
    ...
    result = call_llm(prompt=system_prompt, temperature=0.7, max_tokens=100, timeout=10)
    if result:
        return result
    print(f"[问候] LLM 返回空")
    return "回来啦。"
...
def generate_tease():
    ...
    from core.llm_client import call_llm
    result = call_llm(prompt=system_prompt, temperature=0.8, max_tokens=100, timeout=10)
    if result:
        return result
    print(f"[调侃] LLM 返回空")
    return "你还知道回来？"
...
def generate_proactive(trigger_type: str, context: dict = None):
    ...
    # ─── 构建 prompt（按优先级融合）───
    prompt_parts = [personality_part]
    if base_rules:
        prompt_parts.append(base_rules)
    if tone_hint:
        prompt_parts.append(tone_hint)
    if trust_hint:
        prompt_parts.append(trust_hint)
    if cross_hint:
        prompt_parts.append(cross_hint)
    if trait_hint:
        prompt_parts.append(trait_hint)
    if style_hint:
        prompt_parts.append(style_hint)
    if memory_context:
        prompt_parts.append(memory_context)
    if emotion_summary:
        prompt_parts.append(f"用户近期情绪：{emotion_summary}")
    prompt_parts.append(time_context)
    prompt_parts.append(idle_desc)
    prompt_parts.append(f"请用一句简短自然的话（10-20字）主动对{user_name}说话。{trigger_instruction}。直接说内容，别加动作描写。")

    prompt = "\n".join(prompt_parts)

    from core.llm_client import call_llm
    result = call_llm(prompt=prompt, temperature=0.8, max_tokens=80, timeout=10)
    if result:
        return result
    print(f"[主动] LLM 返回空")
    return None