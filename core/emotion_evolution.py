...
_last_evolution_date = None  # 内存标记，防止同一天重复触发


def process_daily_evolution():
    """每日触发一次。检查所有时间维度演化规则。"""
    global _last_evolution_date
    today = datetime.now().strftime("%Y-%m-%d")
    if _last_evolution_date == today:
        return
    _last_evolution_date = today
...
def _get_hours_since_last_interaction() -> float | None:
    ...
    try:
        import os, json
        last_file = "data/last_chat.json"
        if os.path.exists(last_file):
            with open(last_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                last = datetime.fromisoformat(data.get("last_time", "2000-01-01T00:00:00"))
...
def _check_sustained_low_mood():
    ...
    if consecutive_negative >= 3:
        ...
        if not existing or existing != str(datetime.now().strftime("%Y-%m-%d")):
...
def _rule_silence_decay(hours_since: float | None):
    ...
    print(f"[情绪演化] 沉默衰减: {int(days)}天 → affection{affection_loss:+.1f} trust{trust_loss:+.1f}")
