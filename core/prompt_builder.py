...
def build_system_prompt(current_time_str, emotion="neutral", user_message="", rag_context="", summary="", keypoints=None, custom_context="", tiered_summaries=None, user_patterns=None, sentence_mode="auto"):
    ...
    recall_past = personality.get("recall") or get_config("recall_past", "从不")
    ...
    if custom_prompt and custom_prompt.strip():
        personality_part = custom_prompt.strip() + f" {length_rule}。"
        ...
        emotion_note = "可以在自然的时候用颜文字表达情绪，比如（笑）（叹气）（歪头）这种，别用emoji图标。也可以用感叹号" if allow_emotion else "不要用感叹号，不要用表情符号"
        if tone == "自由":
            personality_part = f"你是佐仓。你可以根据对话情境自由调整语气风格，不必拘泥于单一语气。{length_rule}。{emotion_note}"
        else:
