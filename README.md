...
> **v0.5.0 已发布** — 24 项 Bug 修复 + 多项体验优化。**不想装 Python？** 下载[打包版](#方式一exe-一键运行推荐新手)双击即用，零环境依赖。
...
从 [Releases](https://github.com/kuiti/Vesper/releases) 下载最新版 `Vesper_v0.5.0_packaged.zip`。
...
### v0.5.0（2026-05-25）—— 质量与体验更新

- **24 项 Bug 修复**全新审计的四轮全量代码审计
- 修复 5 项崩溃级：FTS5 搜索 500 错误、schedule/achievements/ai_diary 表缺失、向量重建 numpy 崩溃、LockScreen 长 PIN 锁死、天气推送每日仅一次
- 修复 8 项高危：recall_rule 丢失导致记忆回忆失效、情感状态机 or→and 误判、高德 Key 永远不回显、passphrase 明文泄露、备份导出含 api_key、系统通知开关无效、geolocation 每次重弹权限、recall_past 设置不生效
- 修复 8 项中危：XSS 双引号未转义、WeatherCard null 渲染异常、意图误匹配新闻搜索、TTS 同步阻塞、saveCfg 竞态、逐字模式丢前缀/空气泡、自定义 prompt 跳过 length_rule
- **好感度/信任度双向牵制**：信任越高好感涨得越快，反之亦然，7档渐进乘数
- **性格演化门槛大幅降低**：从"月"级别缩短到"天"级别
- 主动消息融入天气和位置：问候时自然提及当地天气
- 通知风格扩展为 6 种（温暖/随意/幽默/简洁/傲娇/自由）
- 语气新增"自由"选项（AI 根据情境自行调整）
- 兜底问候语扩充为每时段 5 条随机选取
- LLM 问候增加 3 次重试 + 30s 超时容错
- 聊天消息时间显示相对时间（"3分钟前"/"昨天 14:30"）
- 迷你关系状态栏（聊天顶部实时显示好感度/信任度）
- Notes/Countdowns 新增编辑端点、Goals 新增创建/删除端点

详见 [RELEASE_v0.5.0.md](RELEASE_v0.5.0.md)

### v0.4.0（2026-05-23）—— 重大稳定性更新

- **86 项 Bug 修复**全新审计的全量代码审计
